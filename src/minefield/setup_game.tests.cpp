...
{
    auto getOutputofReading
        = [](unsigned int min, unsigned int max, unsigned int value) { // gets the console output when an input string is sent to the readIntInRange function
              std::string input = "";
              ...
              std::istringstream fakeInput(
                  input); // the input string consists of of entering a value, then entering another value of the first value is not withing the specified range
    ...
    EXPECT_NE(getOutputofReading(-1, 5, 3).find(expectedErrorMessage), std::string::npos);
    EXPECT_NE(getOutputofReading(5, 0, 9).find(expectedErrorMessage), std::string::npos);
}

TEST(SETUP, game_is_set_up_correctly)
{ // made by agus to guide me, thanks agus!
    std::istringstream fakeInput;
    std::ostringstream fakeOutput;
...
TEST(SETUP, board_does_not_accept_invalid_values)
{
    auto buildInputString = [](int width, int height) { // build the string of player inputs for the game width and height definitions
        auto buildDimensionString = [](int dimension) { // build the string of player inputs for each dimension
            ...
            if (dimension > MAX_BOARD_SIZE
                || dimension < MIN_BOARD_SIZE) // if this is the case, the program will output an error message and will ask for input again
    ...
    std::string expectedErrorMessage = "Too wild! Choose something in the safe zone";
    EXPECT_EQ((getOutputWH(MAX_BOARD_SIZE, MAX_BOARD_SIZE)).find(expectedErrorMessage),
        std::string::npos); // there will be no error message if width and height are valid values
